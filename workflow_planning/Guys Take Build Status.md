...
| Enforcement hooks (lint Airtable trigger fields, Telegram record-ID, manual-edit guard, secret blocking) | `.claude/hooks/` |
...
12. **Add config-level `Retry On Fail`** (sibling of `onError`) to critical external calls; the host hits transient `EAI_AGAIN` DNS failures.
...
15. **Manual-edit guard:** a PreToolUse hook blocks `update_workflow` if Jonny edited the workflow in the n8n UI since the repo last synced; pull `get_workflow_details` and reconcile first.
...
| 5+6+7 | Frame.io upload → transcript → AI metadata | Frame.io `file.ready` webhook | → `AI Analysis Complete` | Gemini transcription, GPT 5.1 titles/captions/description/chapters, Telegram doc. `_Bypass` + same-length short-circuits | ✅ `jroXHciDvy0sWlRM` — LIVE since 2026-06-16; audio uploads also transcribed (2026-06-29) |
| 8 | Artwork | Thumbnails: `Ready for 16X9 Thumbnail` view (generate) + `Selected` view (pick) + **`Revision Requested` view (revise)** | → `Artwork Ready` | Type-aware (Take/Chat/Clip/**Roundtable** all auto). 4× Nano Banana 16:9; prompts in Airtable `Prompts` table; on pick: reject+unlink siblings, 1:1 + 9:16 reversions (Phase B); **Path 3: briefed in-place targeted revisions** (2026-07-26) | ✅ **`mLAn4ya2AmZoHDUk`** — **ACTIVE**, SDK-sourced (`guys-take-thumbnail-artwork.sdk.js`), old `YyXiJ0lusoW7ynu9` archived. Phase B + Path-3 revisions tested. ⚠️ Roundtable auto-gen still **UNTESTED** |
